...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['selected_membership'])) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($result->num_rows > 0) {
                ...
                if (time() < $expirationDate) {
                    // Membership is still active, user cannot select a new one
        ...
        // Delete user's record from the membership table
...
// Check if the user has a package and notify
$stmt = $conn->prepare("SELECT selected_package FROM package WHERE email = ?");
$stmt->bind_param("s", $loggedInEmail);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows > 0) {
    echo "You have an active package. You cannot select a membership.";
}
