...
$purchasedPackageResult = $conn->query("SELECT COUNT(DISTINCT email) AS purchased_package FROM package WHERE selected_package IS NOT NULL AND email != '$admin_email'");
...
$selectedMembershipResult = $conn->query("SELECT COUNT(DISTINCT email) AS selected_membership FROM membership WHERE selected_membership IS NOT NULL AND email != '$admin_email'");
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $membership_cost = isset($membership_prices[$row['selected_membership']]) ? $membership_prices[$row['selected_membership']] : 0;
        $package_cost = isset($package_prices[$row['selected_package']]) ? $package_prices[$row['selected_package']] : 0;

        // Calculate sales totals
        $total_package_sales += $package_cost;
        $total_membership_sales += $membership_cost;
    }
}
